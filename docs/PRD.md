...
- **Payments sequencing — the remaining open policy question.** The reservation saga bounds the charge-vs-commit divergence to the safe direction. The remaining decision is the **paid-but-expired** policy: if a buyer's UPI approval lands microseconds after the reaper expires their hold and capacity is now gone, do we (a) auto-refund, or (b) hold a small over-capacity buffer to honor them? §4.2 assumes (a); confirm with product before build.
- **Reservation TTL tuning.** Too short and legitimate UPI payers (who can take minutes to approve in-app) lose their hold; too long and inventory is starved during a hot on-sale. A sensible default (10–15 min) is a knob, not a constant, and should differ by payment method if data supports it.
